 ...
 *
 * Routes:
 *   GET  /api/institutions              — PROGRAMME_MANAGER lists all institutions
 *   POST /api/institutions              — PROGRAMME_MANAGER creates an institution
 *   GET  /api/institutions/:id/summary  — PROGRAMME_MANAGER gets per-batch breakdown
 */
...
function addAttendanceRate<T extends Record<string, unknown>>(row: T) {
 ...
 * PROGRAMME_MANAGER lists all institutions in the system.
 */
institutionsRouter.get('/', requireRole('PROGRAMME_MANAGER', 'MONITORING_OFFICER'), async (c) => {
  const result = await db.query.institutions.findMany({
    orderBy: (inst, { asc }) => [asc(inst.name)],
  })
  return c.json({ institutions: result })
})

/**
 * POST /api/institutions
 * PROGRAMME_MANAGER creates a new institution.
 */
institutionsRouter.post('/', requireRole('PROGRAMME_MANAGER'), async (c) => {
  ...
  const [institution] = await db
    .insert(institutions)
    .values({ name: parsed.data.name, region: parsed.data.region })
    .returning()
 ...
 * PROGRAMME_MANAGER gets per-batch attendance breakdown for a specific institution.
 */
institutionsRouter.get('/:id/summary', requireRole('PROGRAMME_MANAGER', 'MONITORING_OFFICER'), async (c) => {
  const institutionId = c.req.param('id')

  const institution = await db.query.institutions.findFirst({
    where: eq(institutions.id, institutionId),
  })
  if (!institution) return notFound(c, 'Institution not found.')
  ...
  return c.json({
    institution,
    batches: batches.map((row: Record<string, unknown>) => addAttendanceRate(row)),
  })
})
